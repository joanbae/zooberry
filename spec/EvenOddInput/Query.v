...
Require Import Monad vgtac VocabA Syn InterCfg DStr DLat DItv DPos
        UserInputType.
Require Import DomBasic DomArrayBlk DomAbs DomMem SemEval SemAbs Zparity.
...
Definition check_bo arr i : list status :=
  if ArrayBlk.eq_dec arr ArrayBlk.bot then BotAlarm :: nil else
    let check_bo' a ofs_size lst :=
      ...
      let ofs := Zparity.plus ofs i in
      let status :=
        match ofs, size with
        | Zparity.Bot, _
        | _, Zparity.Bot => BotAlarm
        | _, _ => UnProven
        end in
      status :: lst in
...
: m (list status) :=
  ...
  | ArrayExp lv e =>
    ...
    ret (check_bo (array_of_val v1) (zparity_of_val v2))
  | DerefExp e =>
    do v <- eval mode node e mem ;
    let statuses := check_bo (array_of_val v) Zparity.top in
    let r :=
