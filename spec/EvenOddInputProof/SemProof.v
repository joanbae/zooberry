...
Definition Zparity_g := Zparity.gamma.

Inductive ArrayBlk_g' : DomCon.Region.t -> ArrayBlk.t -> Prop :=
| ArrayBlk_g_intro :
    forall s a o sz st o' sz' st' ab
           (Ho : Zparity_g o o') (Hsz : Zparity_g sz sz' ) (Hst : Zparity_g st st')
...
(*value is integer*)
| Val_g_z :
    forall z i ls ab ps (Hz : Zparity_g z i), Val_g' (inl (inl z)) (i, ls, ab, ps)
(* value is location *)
| Val_g_loc :
...
- by apply Zparity.gamma_monotone with o'.
- by apply Zparity.gamma_monotone with sz'.
- by apply Zparity.gamma_monotone with st'.
- by auto.
Qed.
...
inversion Hx; subst.
- apply Val_g_z. by apply Zparity.gamma_monotone with i.
...
Lemma not_Zeven_is_Zodd :
  forall n : Z, ~Zeven n -> Zodd n.
Proof.
  intros. destruct n.
- s. apply H. apply I.
- remember (Z.pos p) as k. Admitted.
...
Proof.
  destruct 1.
  - constructor.  
    by apply Zparity.cor_zparity_top.
  - s.
    destruct (Zeven_dec z).
    * constructor. apply Zparity.G_Even. apply z0.
    * constructor. apply Zparity.G_Odd. apply not_Zeven_is_Zodd, n.
  - s.
    destruct (Zeven_dec c).
    * constructor. apply Zparity.G_Even, z.
    * constructor. apply Zparity.G_Odd, not_Zeven_is_Zodd,  n.
  - s. constructor. apply Zparity.G_Top.
  - s. constructor. apply Zparity.G_Top.
Qed.
...
Lemma neg_Even_is_Even : forall z : Z, Zeven z -> Zeven (- z).
Proof.  i. destruct z; by auto. Qed.

Lemma neg_Odd_is_Odd : forall z : Z, Zodd z -> Zodd (- z).
Proof. i. destruct z; by auto. Qed.
...
Proof.
  ...
  - inversion Habs; subst. constructor.
    rewrite <- Z.sub_0_l.
    assert (Hgoal : (Zparity_g (0 - z) (Zparity.minus Zparity.Even i))).
    { destruct i; inversion Hz; subst; s;  try (by apply Zparity.G_Top).
      apply Zparity.G_Even. apply neg_Even_is_Even. apply H.
      apply Zparity.G_Odd.  apply neg_Odd_is_Odd. apply H. }
     destruct i; s in Hgoal
               ; [by inversion Hz|by constructor|by auto|by auto] .
  - inversion Habs; subst. constructor.
    unfold Zparity.b_not_zparity.
    apply Zparity.unknown_unary_prop; i.
    inversion Hz; subst; by inversion FH.
  - inversion Habs; subst. constructor.
    eapply Zparity.not_zparity_prop1; [by apply Ht| by auto].
  - inversion Habs; subst. constructor.
    eapply Zparity.not_zparity_prop2; by auto.
Qed.

Lemma zparity_non_bot :
  forall z abs_v (Habs : Val_g (DomCon.val_of_z z) abs_v),
    ~ (Zparity.eq (zparity_of_val abs_v) Zparity.bot).
Proof.
i. destruct abs_v as [[[i ls] ab] ps].
inversion Habs; subst.
eapply Zparity.non_bot; [by apply Hz|by apply FH].
Qed.

(*Non sense*)
(* Lemma itv_non_zero :
  forall z abs_v (Ht : z <> 0%Z) (Habs : Val_g (DomCon.val_of_z z) abs_v),
    ~ (Zparity.eq (zparity_of_val abs_v) Zparity.zero).
Proof.
  
inversion 2; subst. s; i.
exploit Zparity.gamma_mor; [reflexivity|by apply FH|by apply Hz|].
inversion 1; subst.
elim Ht. 
inversion Ht. inversion Hle1. inversion Hle2. omega.
Qed.
*)
...
Lemma cor_plus_offset :
  forall step alloc o sz st z ab i
     (Hab : ArrayBlk_g (step, alloc, (o, sz, st)) ab) (Hz : Zparity_g z i),
    ArrayBlk_g (step, alloc, ((o + z)%Z, sz, st)) (ArrayBlk.plus_offset ab i).
Proof.
inversion 1; subst; i.
apply ArrayBlk_g_intro with (o':=Zparity.plus o' i) (st':=st') (sz':=sz')
; [by apply Zparity.cor_plus|by auto|by auto|].
unfold ArrayBlk.plus_offset.
destruct (Zparity.eq_dec i Zparity.bot)
; [exploit Zparity.non_bot; [by apply Hz|by apply e|by auto]|].
erewrite ArrayBlk.map_1; [| |by apply Hab0].
- unfold ArrInfo.plus_offset.
  destruct (Zparity.eq_dec Zparity.bot o'); [|reflexivity].
  exploit Zparity.non_bot; [by apply Ho|by apply Zparity.eq_sym|by auto].
- unfold ArrInfo.plus_offset. simpl.
  destruct (Zparity.eq_dec Zparity.bot Zparity.bot)
  ; [by auto|elim f0; by apply Zparity.eq_refl].
Qed.
...
Proof.
inversion 1; subst; i.
apply ArrayBlk_g_intro with (o':=Zparity.minus o' i) (st':=st') (sz':=sz')
; [by apply Zparity.cor_minus|by auto|by auto|].
...
Lemma c_mod_even :
  forall z1 z2 (Hz1 : Zeven z1) (Hz2 : Zeven z2),
    Zeven (SemCon.c_mod z1 z2).
Proof.
  i. unfold SemCon.c_mod.
  apply Zparity.Zeven_minus_Zeven; [by auto|].
...
inversion_clear Habs1; inversion_clear Habs2. by apply Zparity.cor_plus.
}
{                               (* PlusPI *)
...
constructor; by apply Zparity.cor_minus.
}
{                               (* MinusPI *)
i. unfold SemEval.eval_bop.
inversion_clear Habs2; subst.
...
by apply Zparity.times_prop.
}
{                               (* Div *)
i. unfold SemEval.eval_bop. constructor.
inversion_clear Habs1; inversion_clear Habs2.
by apply Zparity.divide_prop.
}
{                               (* Mod *)
i. unfold SemEval.eval_bop, Zparity.mod_zparity.
inversion_clear Habs1 as [z1' i1' ls1' ab1' ps1' Hz1'| | |].
inversion_clear Habs2 as [z2' i2' ls2' ab2' ps2' Hz2'| | |].
